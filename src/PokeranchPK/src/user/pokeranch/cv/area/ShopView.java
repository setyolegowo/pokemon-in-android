...
public class ShopView extends SurfaceView implements SurfaceHolder.Callback {

	private static int screenWidth;
	private static int screenHeight;
	public static float density;
	
	private ShopViewListener mShopViewListener;
	...
	public ShopView(Context context) {
		...
		hero.setStepped(shop.Stepped());
		hero.setOffset(shop.getXOffset(), shop.getYOffset());
		mShopViewListener = (ShopViewListener) context;
			...
			switch (actioncode) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					if((delX > margin_move) && (delY < margin_tolerance) && (delY > -margin_tolerance)) {
						...
						// Bawah
						if(isToChangeView() == 1) { mShopViewListener.onOutToCity(callAct); callAct++; }
						hero.Bawah();
					}
					break;
				case MotionEvent.ACTION_UP:
					...
					if((abs(x - event.getX()) < margin_move) && (abs(y - event.getY()) < margin_move)) {
						...
						if(isFrontShop() == 1) { mShopViewListener.startInteract(callAct); callAct++; }
					}
					break;		
			}
		}
	...
	private int isFrontShop() {
		if((hero.getX() == 17) && (hero.getY() == 5)) return 1; // City
		else
			return 0;
	}
