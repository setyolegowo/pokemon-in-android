...
public class BattleOuterArea extends Activity implements BattleView.BattleViewListener {

	private static final String TAG = BattleOuterArea.class.getSimpleName();
	...
	public static final String FLOOR = "floor_battle";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		current_floor = intent.getIntExtra(FLOOR, 1);
		if(intent.getStringExtra(Main.EXTRA_MESSAGE).equals(ShoreActivity.class.getSimpleName())) {
			IsAtShore = true;
		} else {
			IsAtShore = false;
		}
	...
	private void init_battle_content() {
		...
		if(!IsAtShore)
			monster = Monster.getRandomMonsterOuterArea(db.getReadableDatabase(), current_floor, Main.player.getDayOrNight());
		else
			monster = Monster.getRandomMonsterLaut(db.getReadableDatabase(), 1, 0);
		...
		BattleView.player_picture(PlayerMonster.getPlayerMonster(PlayerParty.getMonsterIndex(monster_party_number)).getMonsterNumber());
	...
	private void battle_content() {
        ...
        battle.PlayerPicture(PlayerMonster.getPlayerMonster(PlayerParty.getMonsterIndex(monster_party_number)).getMonsterNumber());
        ...
        if((number_interaction < 3) || UseItem.isUseItem) {
	...
	public void onResume() {
		super.onResume();
		if(UseItem.isUseItem) {
			if(UseItem.ball_use == 0) {
				interact.setText(playerMonster.getName() + " \nmenggunakan item");
			...
			if(battle.isWaitEvent()) {
				if((number_interaction < 3) && (skill_use == 0)) {
					...
					if(foeMonster.getCurHP() == 0) {
						...
						if(money_get == 0) {
							Log.d(TAG, "MONEY GET = " + money_get);
							money_get = foeMonster.getLevel()*foeMonster.getMaxHP();
							Main.player.setCurMoney(Main.player.getCurMoney() + money_get);
						}
						Log.d(TAG, "Interaction " + number_interaction);
						interact(number_interaction);
						number_interaction++;
	...
	private class BagListener implements View.OnClickListener {
		...
		public void onClick(View arg0) {
			...
			Intent intent = new Intent(getApplicationContext(), UseItem.class);
			startActivity(intent);
		}
		
	}
	
	private class EscapeListener implements View.OnClickListener {

		@Override
		public void onClick(View arg0) {
			// TODO Auto-generated method stub
			if(try_escape*playerMonster.getLevel()*32/(foeMonster.getLevel()/4 + 30) > 150) {
	...
	private void using_skill() {
		...
		if(useSkill) {
			...
			skill_use_turn = 1;
			interaction_skill_use = 1;
			if(foeMonster.getSpeed() < playerMonster.getSpeed())
	            IsPlayerFirst = true;
	        else
	            IsPlayerFirst = false;
			use_skill();
	...
	private void FailedEscape() {
		interact.setText(foeMonster.getName() + " menolak untuk membiarkan \n" + playerMonster.getName() + " untuk kabur");
		boxFront.setVisibility(View.GONE);
		IsPlayerFirst = true;
		skill_use_turn = 2;
		interaction_skill_use = 1;
		...
		if((!IsPlayerFirst && skill_use_turn == 1) || (IsPlayerFirst && skill_use_turn == 2)) {
			...
			switch(interaction_skill_use) {
				...
				case 2: // Target PLAYER HP
					...
					PlayerMonster player_monster = PlayerMonster.getPlayerMonster(PlayerParty.getMonsterIndex(monster_party_number));
					player_monster.setCurHP(playerMonster.getCurHP());
                    ...
                    if(foeSkill[monster_skill_use - 1].getStatusEffect() > 0) {
                        if((foeSkill[monster_skill_use - 1].getProbStatEffect()*((Math.random()*20) + 95)/100) > 100) {
                            if(playerMonster.getNoStatus() == 0){
                                pending_effect = true;
                                player_monster.setStatusEffect(foeSkill[monster_skill_use - 1].getStatusEffect());
                            }
                        }
                    }
                    PlayerMonster.updatePlayerMonster(player_monster);
					break;
	...
	public void isMonsterCaught(int callAct) {
		Log.d(TAG, "CallAct monster caught" + callAct);
		if(callAct == 1) {
			if((((100*(3*foeMonster.getMaxHP() - 2*foeMonster.getCurHP())*((int) Math.random()*10))/7)/(3*foeMonster.getMaxHP()*UseItem.ball_use )) < 1000) {
				interact.setText("Berhasil menangkap monster!");
				Toast.makeText(getApplicationContext(), "Trapped!", Toast.LENGTH_LONG).show();
				BattleView.isMonsterCaught = true;
				Intent intent = new Intent(this, SaveMonster.class);
				intent.putExtra(SaveMonster.FOE_ID, foe_id);
				intent.putExtra(SaveMonster.LEVEL, foeMonster.getLevel());
				startActivity(intent);
			} else {
				Toast.makeText(getApplicationContext(), "Failed!", Toast.LENGTH_LONG).show();
				interact.setText("Gagal menangkap monster!");
				BattleView.isUseMonsterBall = false;
				BattleView.isMonsterCaught = false;
			}
			battle.resetCallAct();
		}
	}
	...
	private class MonsterBattle {
		...
		public MonsterBattle(Monster monster) {
			...
			if(current_floor > 1) 
				this.Level = (10*(current_floor-1)) + (generator2.nextInt(10) + 1);
			else
				this.Level = (generator2.nextInt(5) + 1);
